...
from lvmtarget import desi_mask, bgs_mask, mws_mask, targetid_mask
...
def encode_targetid(objid=None,brickid=None,release=None,mock=None,sky=None):
    ...
    :class:`int` or `~numpy.ndarray`
        ...
        :meth:`lvmtarget.targetid_mask`. If an integer is passed, then an
    ...
    #ADM populate TARGETID based on the passed columns and lvmtarget.targetid_mask
...
def calc_priority(targets):
    ...
    Args:
        targets: numpy structured array or astropy Table of targets, including
            columns LVM_TARGET, BGS_TARGET, and MWS_TARGET
    ...
    if 'LVM_TARGET' in targets.colnames:
        for name in ('ELG', 'LRG'):
            ii                   = (targets['LVM_TARGET'] & desi_mask[name]) != 0
    ...
    ii = (targets['LVM_TARGET'] & desi_mask.IN_BRIGHT_OBJECT) != 0
    priority[ii] = -1
...
def calc_numobs(targets):
    ...
    Args:
        targets: numpy structured array or astropy Table of targets, including
            columns LVM_TARGET, DECAM_FLUX, and DECAM_MW_TRANSMISSION
    ...
    islrg = (targets['LVM_TARGET'] & desi_mask.LRG) != 0
    ...
    isqso = (targets['LVM_TARGET'] & desi_mask.QSO) != 0
    nobs[isqso] = 4
