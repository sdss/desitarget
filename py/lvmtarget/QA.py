...
from lvmutil.plots import init_sky, plot_sky_binned
...
def remove_fractional_bricks(targs,frac=0.9,bricksize=0.25):
    ...
    targs : :class:`~numpy.ndarray`
        File of targets generated by `lvmtarget.bin.select_targets`
...
def remove_fractional_pixels(targs,frac=0.9,nside=256):
    ...
    ----------
    targs : :class:`~numpy.ndarray`
        File of targets generated by `lvmtarget.bin.select_targets` must contain a column
...
def is_polygon_within_boundary(boundverts,polyverts):
    ...
    Testing the vertices should be good enough for lvmtarget QA, though.
    """
...
def mag_histogram(targetfilename,binsize,outfile):
    ...
    targetdata = fx[1].read(columns=['BRICKID','LVM_TARGET','BGS_TARGET','MWS_TARGET','DECAM_FLUX'])
    ...
    for i, bitval in enumerate(bitvals):
        ...
        w = np.where(targetdata["LVM_TARGET"] & bitval)
        if len(w[0]):
...
def populate_HPX_info(instruc,hpxids,nside=256):
    ...
    from lvmmodel import footprint
    import healpy
...
def HPX_info(targetfilename,outfilename='hp-info-dr3.fits',nside=256):
    ...
    from lvmmodel import footprint, io
    ...
    #ADM bits and names of interest for lvmtarget
...
def brick_info(targetfilename,rootdirname='/global/project/projectdirs/cosmo/data/legacysurvey/dr3/',outfilename='brick-info-dr3.fits'):
    ...
    log.info('Reading in target file...t = {:.1f}s'.format(time()-start))
    fx = fitsio.FITS(targetfilename, upper=True)
    targetdata = fx[1].read(columns=['BRICKID','LVM_TARGET','BGS_TARGET','MWS_TARGET'])
...
def _load_targdens():
    """Loads the target info dictionary as in :func:`lvmmodel.io.load_target_info()` and
    extracts the target density information in a format useful for targeting QA plots
    """

    from lvmmodel import io
...
def qaskymap(cat, objtype, qadir='.', upclip=None, weights=None, max_bin_area=1.0, fileprefix="skymap"):
    ...
    shamelessly from :mod:`lvmtarget.mock.QA` (which was originally written by `J. Moustakas`)
...
def qahisto(cat, objtype, qadir='.', targdens=None, upclip=None, weights=None, max_bin_area=1.0,
    ...
    #ADM the HEALPixel number for each RA/Dec (this call to lvmmodel
...
def qacolor(cat, objtype, qadir='.', fileprefix="color"):
    ...
    from lvmtarget.cuts import unextinct_fluxes
...
def make_qa_plots(targs, qadir='.', targdens=None, max_bin_area=1.0, weight=True):
    ...
    weight : :class:`boolean`, optional, defaults to True
        If this is set, weight pixels using the ``LVMMODEL`` HEALPix footprint file to
    ...
    Notes
    -----
    The ``LVMMODEL`` environment variable must be set to find the file of HEALPixels
    that overlap the DESI footprint
    """

    #ADM set up the default logger from lvmutil
    from lvmutil.log import get_logger, DEBUG
    log = get_logger(DEBUG)
    from lvmmodel import io, footprint
    ...
    #ADM Current goal target densities for DESI (read from the LVMMODEL defaults)
    if targdens is None:
    ...
    for objtype in targdens:
        if 'ALL' in objtype:
            ...
            w = np.where(targs["LVM_TARGET"] & desi_mask[objtype])
...
def make_qa_page(targs, makeplots=True, max_bin_area=1.0, qadir='.', weight=True):
    ...
    Notes
    -----
    If making plots, then the ``LVMMODEL`` environment variable must be set to find
    ...
    #ADM the goal target densities for DESI (read from the LVMMODEL defaults)
    targdens = _load_targdens()
