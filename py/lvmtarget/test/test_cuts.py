...
class TestCuts(unittest.TestCase):
    ...
    def _test_table_row(self, targets):
        ...
        self.assertTrue(isinstance(desi, numbers.Integral), 'LVM_TARGET mask not an int')
    ...
    def test_parallel_select(self):
        for nproc in [1,2]:
            for filelist in [self.tractorfiles, self.sweepfiles]:
                ...
                self.assertTrue('LVM_TARGET' in targets.dtype.names)
                self.assertTrue('BGS_TARGET' in targets.dtype.names)
                self.assertTrue('MWS_TARGET' in targets.dtype.names)
                self.assertEqual(len(targets), np.count_nonzero(targets['LVM_TARGET']))

                bgs1 = (targets['LVM_TARGET'] & desi_mask.BGS_ANY) != 0
