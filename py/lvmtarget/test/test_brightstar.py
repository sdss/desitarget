...
from lvmtarget import brightstar, desi_mask, targetid_mask, io
...
class TestBRIGHTSTAR(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.bsdatadir = resource_filename('lvmtarget.test', 't2')
        ...
        #ADM because the input file has not been through Target Selection we need to add LVM_TARGET and TARGETID
        zeros = np.zeros(len(masktargs))
        masktargs = rfn.append_fields(masktargs,["LVM_TARGET","TARGETID"],
        ...
        unmasktargs = rfn.append_fields(unmasktargs,["LVM_TARGET","TARGETID"],
                                        [zeros,zeros],usemask=False,dtypes='>i8')
        #ADM As the sweeps file is also doubling as a targets file, we have to duplicate the
        #ADM column "BRICK_OBJID" and include it as the new column "OBJID"
    ...
    def test_mask_targets(self):
        ...
        self.assertTrue(np.any(targs["LVM_TARGET"] != 0))
    ...
    def test_non_mask_targets(self):
        ...
        self.assertTrue(np.all((targs["LVM_TARGET"] == 0) | ((targs["LVM_TARGET"] & desi_mask.BADSKY) != 0)))
    ...
    def test_targetid(self):
        """Test SKY/RELEASE/BRICKID/OBJID are set correctly in TARGETID and LVM_TARGET for SAFE/BADSKY locations
        """
        ...
        skybitset = ((targs["TARGETID"] & targetid_mask.SKY) != 0)
        badskybitset = ((targs["LVM_TARGET"] & desi_mask.BADSKY) != 0)
