...
class SelectTargets(object):
    ...
    lvmtarget.cuts but that code has not been factored in a way that is
    convenient at this time.

    """
    def __init__(self, logger=None, rand=None, brick_info=None):
        from lvmtarget import desi_mask, bgs_mask, mws_mask, contam_mask, obsconditions
    ...
    def _star_select(self, targets, truth):
        ...
        """
        from lvmtarget.cuts import isBGS_faint, isELG, isLRG_colors, isQSO_colors
        ...
        targets['LVM_TARGET'] |= (bgs_faint != 0) * self.desi_mask.BGS_ANY
        ...
        targets['LVM_TARGET'] |= (elg != 0) * self.desi_mask.ELG
        targets['LVM_TARGET'] |= (elg != 0) * self.desi_mask.ELG_SOUTH
        ...
        targets['LVM_TARGET'] |= (lrg != 0) * self.desi_mask.LRG
        targets['LVM_TARGET'] |= (lrg != 0) * self.desi_mask.LRG_SOUTH
        ...
        targets['LVM_TARGET'] |= (qso != 0) * self.desi_mask.QSO
        targets['LVM_TARGET'] |= (qso != 0) * self.desi_mask.QSO_SOUTH
    ...
    def _std_select(self, targets, truth, boss_std=None):
        ...
        from lvmtarget.cuts import isFSTD
        ...
        targets['LVM_TARGET'] |= (fstd != 0) * self.desi_mask.STD_FSTAR
        ...
        targets['LVM_TARGET'] |= (fstd_bright != 0) * self.desi_mask.STD_BRIGHT
    ...
    def bgs_select(self, targets, truth, boss_std=None):
        ...
        """
        from lvmtarget.cuts import isBGS_bright, isBGS_faint
        ...
        targets['LVM_TARGET'] |= (bgs_bright != 0) * self.desi_mask.BGS_ANY
    ...
    def elg_select(self, targets, truth, boss_std=None):
        ...
        from lvmtarget.cuts import isELG, isQSO_colors
        ...
        qso = isQSO_colors(gflux=gflux, rflux=rflux, zflux=zflux, w1flux=w1flux,
                           w2flux=w2flux, optical=False) # Note optical=False!
        targets['LVM_TARGET'] |= (qso != 0) * (elg == 0) * self.desi_mask.QSO
        targets['LVM_TARGET'] |= (qso != 0) * (elg == 0) * self.desi_mask.QSO_SOUTH
    ...
    def lrg_select(self, targets, truth, boss_std=None):
        """Select LRG targets."""
        from lvmtarget.cuts import isLRG_colors, isQSO_colors
        ...
        qso = isQSO_colors(gflux=gflux, rflux=rflux, zflux=zflux, w1flux=w1flux,
                           w2flux=w2flux, optical=False) # Note optical=False!
        targets['LVM_TARGET'] |= (qso != 0) * (lrg == 0) * self.desi_mask.QSO
        targets['LVM_TARGET'] |= (qso != 0) * (lrg == 0) * self.desi_mask.QSO_SOUTH
    ...
    def mws_main_select(self, targets, truth, boss_std=None):
        ...
        def _isMWS_MAIN(rflux):
            """A function like this should be in lvmtarget.cuts. Select 15<r<19 stars."""
            main = rflux > 10**( (22.5 - 19.0) / 2.5 )
            main &= rflux <= 10**( (22.5 - 15.0) / 2.5 )
            return main

        def _isMWS_MAIN_VERY_FAINT(rflux):
            """A function like this should be in lvmtarget.cuts. Select 19<r<20 filler stars."""
        ...
        targets['LVM_TARGET'] |= (mws_main != 0) * self.desi_mask.MWS_ANY
        ...
        targets['LVM_TARGET'] |= (mws_main_very_faint != 0) * self.desi_mask.MWS_ANY
    ...
    def mws_nearby_select(self, targets, truth, boss_std=None):
        ...
        targets['LVM_TARGET'] |= (mws_nearby != 0) * self.desi_mask.MWS_ANY
    ...
    def mws_wd_select(self, targets, truth, boss_std=None):
        ...
        targets['LVM_TARGET'] |= (mws_wd != 0) * self.desi_mask.MWS_ANY
        ...
        targets['LVM_TARGET'] |= (std_wd !=0) * self.desi_mask.mask('STD_WD')
    ...
    def qso_select(self, targets, truth, boss_std=None):
        ...
        from lvmtarget.cuts import isQSO_colors, isELG
    ...
    def sky_select(self, targets, truth, boss_std=None):
        ...
        targets['LVM_TARGET'] |= self.desi_mask.mask('SKY')
