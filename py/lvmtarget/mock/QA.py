...
Generate QA figures from the output of lvmtarget.mock.join_targets_truth.
...
from lvmtarget import desi_mask, bgs_mask, mws_mask, contam_mask

def target_density(cat, nside=128):
    """Determine the target density by grouping targets in healpix pixels.  The code
    below was code shamelessly taken from lvmutil.plot.plot_sky_binned (by
    D. Kirkby).
...
def qa_qso(targets, truth, qadir='.'):
    ...
    these = np.where(targets['LVM_TARGET'] & desi_mask.mask('QSO') != 0)[0]
    dens['QSO_TARGETS'] = target_density(targets[these])

    these = np.where((targets['LVM_TARGET'] & desi_mask.mask('QSO') != 0) * (truth['TRUEZ'] < 2.1))[0]
    dens['QSO_TRACER'] = target_density(targets[these])

    these = np.where((targets['LVM_TARGET'] & desi_mask.mask('QSO') != 0) * (truth['TRUEZ'] >= 2.1))[0]
    dens['QSO_LYA'] = target_density(targets[these])

    these = np.where((targets['LVM_TARGET'] & desi_mask.mask('QSO') != 0) *
...
def qa_elg(targets, truth, qadir='.'):
    ...
    these = np.where(targets['LVM_TARGET'] & desi_mask.mask('ELG') != 0)[0]
    dens['ELG_TARGETS'] = target_density(targets[these])

    these = np.where((targets['LVM_TARGET'] & desi_mask.mask('ELG') != 0) *
                     (truth['TRUEZ'] >= 0.6) * (truth['TRUEZ'] <= 1.6))[0]
    dens['ELG_IS_RIGHTZ'] = target_density(targets[these])

    these = np.where((targets['LVM_TARGET'] & desi_mask.mask('QSO') != 0) * (truth['TRUEZ'] < 0.6))[0]
    dens['ELG_IS_LOZ'] = target_density(targets[these])

    these = np.where((targets['LVM_TARGET'] & desi_mask.mask('QSO') != 0) * (truth['TRUEZ'] > 1.6))[0]
...
def qa_targets_truth(output_dir, verbose=True):
    ...
    if nobj > 0:
        ...
        for obj in ('ELG', 'LRG', 'QSO', 'BGS_ANY', 'MWS_ANY'):
            these = np.where((targets['LVM_TARGET'] & desi_mask.mask(obj)) != 0)[0]
            if len(these) > 0:
    ...
    if nsky > 0:
        ...
        obj = 'SKY'
        these = np.where((sky['LVM_TARGET'] & desi_mask.mask(obj)) != 0)[0]
        if len(these) > 0:
    ...
    if ndark > 0 or nbright > 0:
        ...
        for cat, nn, obj in zip( (stddark, stdbright), (ndark, nbright),
            ...
            if nn > 0:
                these = np.where((cat['LVM_TARGET'] & desi_mask.mask(obj)) != 0)[0]
                if len(these) > 0:
