...
from lvmtarget import desi_mask, bgs_mask, mws_mask, contam_mask, targetid_mask, obsconditions
import lvmtarget.mock.io as mockio
from lvmtarget.mock import sfdmap
...
class BrickInfo(object):
    ...
    def generate_brick_info(self):
        ...
        """
        from lvmutil.brick import Bricks
    ...
    def fluctuations_across_bricks(self, brick_info):
        ...
        """
        from lvmtarget.QA import generate_fluctuations
    ...
    def targetinfo(self):
        """Read target info from LVMMODEL, change all the keys to upper case, and
        append into brick_info.

        """
        import yaml
        with open(os.path.join( os.getenv('LVMMODEL'), 'data', 'targets', 'targets.yaml' ), 'r') as filein:
            td = yaml.load(filein)
        target_lvmmodel = Table()
        for t in td.keys():
            if 'ntarget' in t.upper():
                target_lvmmodel[t.upper()] = td[t]

        return target_lvmmodel
...
def add_mock_shapes_and_fluxes(mocktargets, realtargets=None, random_state=None):
    ...
    for objtype in ('ELG', 'LRG', 'QSO'):
        ...
        ii = np.where((mocktargets['LVM_TARGET'] & mask) != 0)[0]
        jj = np.where((realtargets['LVM_TARGET'] & mask) != 0)[0]
        if len(jj) == 0:
...
def empty_targets_table(nobj=1):
    ...
    for fiberassignment to work are: TARGETID, RA, DEC, LVM_TARGET, BGS_TARGET,
    ...
    targets.add_column(Column(name='LVM_TARGET', length=nobj, dtype='i8'))
...
def get_spectra_onebrick(target_name, mockformat, thisbrick, brick_info, Spectra, dust_dir,
    ...
    if target_name == 'faintstar':
        ...
        keep = np.where(targets['LVM_TARGET'] != 0)[0]
        nobj = len(keep)
...
def targets_truth(params, output_dir='.', realtargets=None, seed=None, verbose=False,
    ...
    from lvmutil import depend
    from desispec.io.util import fitsheader, write_bintable
    from lvmtarget.mock.selection import SelectTargets
    from lvmtarget.mock.spectra import MockSpectra
    from lvmtarget.internal import sharedmem
    from lvmmodel.footprint import radec2pix
    from lvmtarget import obsconditions
    ...
    for source_name in sorted(params['sources'].keys()):
        ...
        if 'density' in params['sources'][source_name].keys():
            ...
            keep = np.where(targets['LVM_TARGET'] != 0)[0]
            #keep = np.where( np.any( ((targets['LVM_TARGET'] != 0), (truth['CONTAM_TARGET'] != 0)), axis=0) )[0]
    ...
    for pixnum in healpixels:
        ...
        for stdsuffix in ('dark', 'bright'):
            ...
            if stdsuffix == 'dark':
                ...
                istd = inpixel & ((targets['LVM_TARGET'] & dark_std_mask) != 0)
            elif stdsuffix == 'bright':
                bright_std_mask = desi_mask.STD_BRIGHT | desi_mask.STD_WD
                istd = inpixel & ((targets['LVM_TARGET'] & bright_std_mask) != 0)
            else:
...
def initialize(params, verbose=False, seed=1, output_dir="./", nproc=1, nside=16, healpixels=None):
    ...
    from lvmtarget.mock.spectra import MockMagnitudes
    from lvmtarget.mock.selection import SelectTargets
    import healpy as hp
...
def read_catalog(source_name, params, log, rand=None, nproc=1, healpixels=None, nside=16, in_desi=True):
    ...
    if bool(source_data):
        if in_desi:
            ...
            tiles = lvmmodel.io.load_tiles()
            if n_obj>0:
                indesi = lvmmodel.footprint.is_point_in_desi(tiles, source_data['RA'], source_data['DEC'])
                for k in source_data.keys():
...
def target_selection(Selection, target_name, targets, truth, nside, healpix_id, seed, rand, log, output_dir):
    ...
    if 'LVM_TARGET' in targets.dtype.names:
        no_target_class &=  targets['LVM_TARGET'] == 0
...
def write_to_disk(targets, truth, skytargets, skytruth, nside, healpix_id, seed, rand, log, output_dir):
    ...
    # Write out the dark- and bright-time standard stars.
        for stdsuffix, stdbit in zip(('dark', 'bright'), ('STD_FSTAR', 'STD_BRIGHT')):
            ...
            istd   = (((targets['LVM_TARGET'] & desi_mask.mask(stdbit)) |
                   (targets['LVM_TARGET'] & desi_mask.mask('STD_WD')) ) != 0)
